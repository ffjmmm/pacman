...
class MyAgent(Agent):
    ...
    def getAction(self, state):
        ...
        if 'actionIndex' not in dir(self):
            ...
            search_result = uniformCostSearch(problem, self.index)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        length = state[1]
        index = state[2]
        for i in range(index + 1, len(target_list)):
            if (x, y) == target_list[i]:
                if len_list[i] > length:
                    target_list[i] = (0, 0)
                    len_list[i] = 0
                else:
                    return False
        target_list[index] = (x, y)
        len_list[index] = length
        return True
    ...
    def getCostOfAction(self, state, action):
        ...
        if action == Directions.NORTH:
            return len([agent for agent in self.agents if agent[1] > y and abs(agent[0] - x) <= self.width // 5]) * 5 + 1
        elif action == Directions.SOUTH:
            return len([agent for agent in self.agents if agent[1] < y and abs(agent[0] - x) <= self.width // 5]) * 5 + 1
        elif action == Directions.WEST:
            return len([agent for agent in self.agents if agent[0] < x and abs(agent[1] - y) <= self.height // 5]) * 5 + 1
        elif action == Directions.EAST:
            return len([agent for agent in self.agents if agent[0] > x and abs(agent[1] - y) <= self.height // 5]) * 5 + 1


def uniformCostSearch(problem, index):
    ...
    while 1:
        ...
        current_len = current_node[3]
        if problem.isGoalState((current_location, current_len, index)):
            target_list[index] = current_location
            len_list[index] = current_len
        ...
        if current_location not in closed:
            ...
            for successor in problem.getSuccessors(current_location):
                p_queue.push((successor[0], current_actions + [successor[1]], current_cost + successor[2], current_len + 1),
