    ...
    def train(self, dataset):
        ...
        while True:
            ...
            for x, y in dataset.iterate_once(1):
                y_ = self.get_prediction(x)
                if nn.as_scalar(y) != y_:
                    flag = 0
...
class DigitClassificationModel(object):
    ...
    def __init__(self):
        ...
        self.input = 784
        self.output = 10
        self.w1 = nn.Parameter(self.input, self.n1)
        self.b1 = nn.Parameter(1, self.n1)
        self.w2 = nn.Parameter(self.n1, self.n2)
        self.b2 = nn.Parameter(1, self.n2)
        self.w3 = nn.Parameter(self.n2, self.n3)
        self.b3 = nn.Parameter(1, self.n3)
        self.w4 = nn.Parameter(self.n3, self.output)
        self.b4 = nn.Parameter(1, self.output)
    ...
    def run(self, x):
        ...
        return nn.AddBias(nn.Linear(x3, self.w4), self.b4)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                grads = nn.gradients(self.get_loss(x, y), [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3,
                                                           self.w4, self.b4])
                self.w1.update(grads[0], -self.learning_rate)
                self.b1.update(grads[1], -self.learning_rate)
                self.w2.update(grads[2], -self.learning_rate)
                ...
                self.w3.update(grads[4], -self.learning_rate)
                self.b3.update(grads[5], -self.learning_rate)
                self.w4.update(grads[6], -self.learning_rate)
